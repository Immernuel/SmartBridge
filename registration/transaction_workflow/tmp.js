...
var configSchema = exports_external.object({
  registryUrl: exports_external.string(),
  chainSelectorName: exports_external.string(),
  receiverContract: exports_external.string(),
  tokenContract: exports_external.string(),
  gasLimit: exports_external.string(),
  authorizedSignerAddress: exports_external.string()
});
var resolveDepositAddress = (nodeRuntime, registryToken, walletAddress, token, network248) => {
  const httpClient = new ClientCapability2;
  const url = `${nodeRuntime.config.registryUrl}/deposit-address/${encodeURIComponent(walletAddress)}` + `?coin=${encodeURIComponent(token)}&network=${encodeURIComponent(network248)}`;
  ...
  if (!ok(resp)) {
    throw new Error(`[SmartBridge] deposit-address failed: status=${resp.statusCode}`);
  }
  const bodyStr = new TextDecoder().decode(resp.body);
  const parsed = JSON.parse(bodyStr);
  if (!parsed.address) {
    throw new Error(`[SmartBridge] No address returned for token=${token} network=${network248}`);
  }
  return { address: parsed.address, tag: parsed.tag ?? "" };
};
var makeOnHttpTrigger = (secretsProvider) => (runtime2, payload) => {
  const triggerData = JSON.parse(new TextDecoder().decode(payload.input));
  if (!triggerData.walletAddress || !triggerData.token || !triggerData.network || !triggerData.amount) {
    throw new Error("Invalid payload: walletAddress, token, network and amount are all required.");
  }
  runtime2.log(`[SmartBridge] Wallet:  ${triggerData.walletAddress}`);
  runtime2.log(`[SmartBridge] Token:   ${triggerData.token}`);
  runtime2.log(`[SmartBridge] Network: ${triggerData.network}`);
  runtime2.log(`[SmartBridge] Amount:  ${triggerData.amount}`);
  const registryToken = secretsProvider.getSecret({ id: "REGISTRY_TOKEN" }).result().value;
  const resolved = runtime2.runInNodeMode(resolveDepositAddress, consensusIdenticalAggregation())(registryToken, triggerData.walletAddress, triggerData.token, triggerData.network).result();
  runtime2.log(`[SmartBridge] ✅ Deposit address: ${resolved.address}`);
  if (resolved.tag)
    runtime2.log(`[SmartBridge] ⚠️ Memo/Tag: ${resolved.tag}`);
  ...
  if (!network248)
    throw new Error(`[SmartBridge] Network not found: ${config.chainSelectorName}`);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const reportPayload = encodeAbiParameters(parseAbiParameters("address token, address recipient, uint256 amount"), [
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`[SmartBridge] ✅ TX: ${txHash}`);
  ...
  throw new Error(`[SmartBridge] Transaction failed: ${writeResult.txStatus}`);
};
var initWorkflow = (config, secretsProvider) => {
  const http = new HTTPCapability;
  return [
    handler(http.trigger({
      authorizedKeys: config.authorizedSignerAddress === "0x0000000000000000000000000000000000000000" ? [] : [
        {
