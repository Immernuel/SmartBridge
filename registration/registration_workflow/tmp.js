...
var registerCredentials = (nodeRuntime, registryToken, payload) => {
  ...
  if (!ok(resp))
    throw new Error(`Registry POST failed: ${resp.statusCode}`);
  return { statusCode: resp.statusCode };
};
var makeOnHttpTrigger = (secretsProvider) => (runtime2, payload) => {
  const raw = JSON.parse(new TextDecoder().decode(payload.input));
  if (!raw.walletAddress || !raw.exchange || !raw.apiKey || !raw.apiSecret) {
    throw new Error("Invalid payload: walletAddress, exchange, apiKey and apiSecret are all required.");
  }
  runtime2.log(`[SmartBridge Register] Wallet:   ${raw.walletAddress}`);
  runtime2.log(`[SmartBridge Register] Exchange: ${raw.exchange}`);
  const registryToken = secretsProvider.getSecret({ id: "REGISTRY_TOKEN" }).result().value;
  const result = runtime2.runInNodeMode(registerCredentials, consensusIdenticalAggregation())(registryToken, raw).result();
